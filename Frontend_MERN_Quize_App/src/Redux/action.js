...
export const getAllUserDataFromBackend = () => (dispatch) => {
  // Dispatching the request action
  dispatch(getAllUserDataRequest());

  // Making a GET request to the server to fetch user data
  axios
    ...
    .catch((err) => {
      // Dispatching the failure action if an error occurs
...
export const postQuizResult = (obj) => (dispatch) => {
  ...
  axios
    ...
    .post(`https://mern-quiz-server-sudhir.onrender.com/api/userData/submitQuiz/${userId}`, {
      quizId,    
      quizResult 
    })
    .then((res) => {
